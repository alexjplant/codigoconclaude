...
export function message(msgs: ModelMessage[], model: Provider.Model, options: Record<string, unknown>) {
  ...
  if (
    options.store !== true &&
    key &&
    ["@ai-sdk/openai", "@ai-sdk/github-copilot"].includes(model.api.npm)
  ) {
...
export function options(input: {
  ...
  if (!input.model.api.id.includes("claude") && input.model.api.npm === "@ai-sdk/anthropic") {
  ...
  */
  ...
  if (model.providerID === "google" || model.api.id.includes("gemini")) {
    ...
    const sanitizeGemini = (obj: any): any => {
      ...
      // `["number","string"]` (emitted by some MCP servers). The Google SDK
